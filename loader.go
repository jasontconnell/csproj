...
func loadPackages(path string) []Package {
	...
	pkgscontent, err := os.ReadFile(pkgcfg)
	if err != nil {
		return packages
	}
...
func loadReferences(contents []byte) ([]Reference, []ProjectReference) {
	...
	for _, m := range pkgmatch {
		...
		ref := Reference{Name: name, Hint: "", FullRef: name, PackageName: name, HasPrivate: false, Version: version, IsPackage: true, IsPackageReference: true}
		refs = append(refs, ref)
	}
